...
#[tauri::command]
pub fn run_onvif_server(window: tauri::Window, app: tauri::AppHandle) -> Result<(), String> {
    ...
    tauri::async_runtime::spawn(async move {
        while let Some(event) = rx.recv().await {
            if let CommandEvent::Stdout(line) = event {
                ...
                if window.emit("message", Some(format!("'{}'", line))).is_err() {
                    eprintln!("Failed to emit message event");
                }

                // Write to stdin of the child process
                if child.write("message from Rust\n".as_bytes()).is_err() {
                    eprintln!("Failed to write to child stdin");
                }
            }
        }
    });
